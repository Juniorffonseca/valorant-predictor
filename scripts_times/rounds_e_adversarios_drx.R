...
ds_adversarios_drx$Resultado <- gsub("[^0-9/ .-]", "", ds_adversarios_drx$Resultado)# Deixando apenas números e "/"
#
ds_adversarios_drx$Resultado <- substr(ds_adversarios_drx$Resultado,
                                        gregexpr("/", ds_adversarios_drx$Resultado)[[1]][1] - 3,
                                        gregexpr("/", ds_adversarios_drx$Resultado)[[3]][1] + 3) # Tirando dados que estão longes das barras ("/")

ds_adversarios_drx <- separate(ds_adversarios_drx, Resultado, c("RW", "RL"), "/") # Transformando a coluna Resultado em RW e RL

ds_adversarios_drx$RL <- sub(" .*", "", ds_adversarios_drx$RL) # Tirando todos os caracteres que estavam à direita
ds_adversarios_drx$RW <- sub("*. ", "", ds_adversarios_drx$RW) # Tirando todos os caracteres que estavam à esquerda

ds_adversarios_drx$RW <- sub(" ", "", ds_adversarios_drx$RW) # Tirando todos os espaços
ds_adversarios_drx$RL <- sub(" ", "", ds_adversarios_drx$RL) # Tirando todos os espaços

ds_adversarios_drx$Resultados <- as.numeric(ds_adversarios_drx$RW) > as.numeric(ds_adversarios_drx$RL) # Criando uma coluna de resultados

ds_adversarios_drx$Resultados <- replace(ds_adversarios_drx$Resultados, ds_adversarios_drx$Resultados == TRUE, 'Win') %>% 
  replace(ds_adversarios_drx$Resultados == FALSE, 'Lose') # Renomeando TRUE para 'Win' e FALSE para 'Lose'
